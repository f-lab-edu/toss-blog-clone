...
class HeaderController extends Controller {
  ...
  render() {
    ...
    this.slashButtonView.render({ content: "SLASH", variant: 'white', suffix: move, color: 'gray' });
    this.simplicityButtonView.render({ content: "SIMPLICITY", variant: 'white', suffix: move, color: 'gray' });
    this.subscribeButtonView.render({ content: "구독하기" });
    this.careerButtonView.render({ content: "채용 바로가기", variant: 'secondary', color: 'gray' });
  }
}
